...
\section{A Case Study on Downstream Misinterpretation: The Case of \citet{DBLP:conf/sigsoft/RayPFD14, DBLP:journals/cacm/RayPDF17}}
\label{sec:appendix-misinterpretation}
...
The debate's persistence reflects two orthogonal challenges---\emph{identification} (can the design distinguish causal effects from confounding?) and \emph{measurement} (are the proxies valid?)---neither of which have been fully resolved in the SE community on the programming language vs. code quality debate as of today.
...
We advocate to address this structural gap by requiring researchers to state their causal question, articulate their theory, and argue for valid identification (Section~\ref{sec:guide}), replacing vague ``correlation vs.\ causation'' disclaimers with transparent, evaluable assumptions.
...
Nothing in this paper diminishes their value.
...
For researchers who ask causal questions, this paper offers a concrete path forward: State the estimand, justify the identification strategy, and assess sensitivity to assumptions (Section~\ref{sec:guide}).
...
The worked example in this paper makes this concrete: The na\"ive cross-sectional estimate ($+911\%$) would suggest transformational, organization-scale productivity gains from AI coding tools, while the design-based DiD estimate ($+32\%$ to $+52\%$) is substantial but qualitatively different, and consistent with the magnitude range reported by controlled experiments on AI coding tools (Section~\ref{sec:triangulation}).
...
We beleive that SE researchers, who are trained to reason about complex systems, dependencies, and graph structures, can learn to construct and defend identification arguments if given the right conceptual framework.
