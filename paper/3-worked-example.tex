...
The figure is the same causal theory as Figure~\ref{fig:ai-dag}---same treatment, same mediator, same outcome, same confounders---now drawn with explicit time indices so that every time-varying covariate appears twice: once in its pre-treatment incarnation and once in its post-treatment incarnation.
...
The causal path runs through the single mediator Code Generation, as motivated in Section~\ref{sec:primer-dags}.
...
\node[cfnode] (domain) at (0, 3.4) {Domain};
\node[cfnode] (org) at (3.2, 3.4) {Organization};
\node[unobs] (skill) at (6.4, 3.4) {Team Skill};
\node[cfnode] (maturity) at (9.6, 3.4) {Proj.\ Maturity};

% Pre-treatment time-varying confounders (second row)
\node[cfnode] (pop_pre) at (0, 2.0) {Popularity$_{t\text{-}1}$};
\node[cfnode] (demand_pre) at (3.2, 2.0) {Demand$_{t\text{-}1}$};
\node[cfnode] (activity_pre) at (6.4, 2.0) {Activity$_{t\text{-}1}$};
\node[cfnode] (team_pre) at (9.6, 2.0) {Team Size$_{t\text{-}1}$};

% Main causal chain (middle)
\node[tnode] (treat) at (1.3, 0.5) {AI Adoption};
\node[mnode] (codegen) at (5.1, 0.5) {Code Generation};
\node[ynode] (outcome) at (8.9, 0.5) {Dev.\ Velocity};

% Post-treatment colliders (bottom row)
\node[cnode] (pop_post) at (0, -1.0) {Popularity$_{t\text{+}1}$};
\node[cnode] (demand_post) at (3.2, -1.0) {Demand$_{t\text{+}1}$};
\node[cnode] (activity_post) at (6.4, -1.0) {Activity$_{t\text{+}1}$};
\node[cnode] (team_post) at (9.6, -1.0) {Team Size$_{t\text{+}1}$};

\begin{pgfonlayer}{bg}
% Structural confounders -> treatment and outcome
\draw[cedge] (domain.south) to[out=-100, in=145] (treat.145);
\draw[cedge] (domain.south) to[out=-70, in=165] (outcome.165);
\draw[cedge] (org.south) to[out=-100, in=120] (treat.120);
\draw[cedge] (org.south) to[out=-80, in=145] (outcome.145);
\draw[cedge] (skill.south) to[out=-100, in=60] (treat.60);
\draw[cedge] (skill.south) to[out=-95, in=115] (outcome.115);
\draw[cedge] (maturity.south) to[out=-120, in=30] (treat.30);
\draw[cedge] (maturity.south) to[out=-95, in=25] (outcome.25);

% Pre-treatment confounders -> treatment and outcome
\draw[cedge] (pop_pre) -- (treat);
\draw[cedge] (pop_pre.east) to[bend left=8] (outcome);
\draw[cedge] (demand_pre) -- (treat);
\draw[cedge] (demand_pre) -- (outcome);
\draw[cedge] (activity_pre) -- (treat);
\draw[cedge] (activity_pre) -- (outcome);
\draw[cedge] (team_pre) -- (treat);
\draw[cedge] (team_pre) -- (outcome);

% Mediator (single)
\draw[medge] (treat) -- (codegen);
\draw[medge] (codegen) -- (outcome);

% Collider paths
\draw[coledge] (treat) -- (pop_post);
\draw[coledge] (outcome) -- (pop_post);
\draw[coledge] (treat) -- (demand_post);
\draw[coledge] (outcome) -- (demand_post);
\draw[coledge] (treat) -- (activity_post);
\draw[coledge] (outcome) -- (activity_post);
\draw[coledge] (treat) -- (team_post);
\draw[coledge] (outcome) -- (team_post);
\end{pgfonlayer}
\end{tikzpicture}
\caption{Temporal DAG for the AI coding tools and development velocity problem, resolving the temporal ambiguity in Figure~\ref{fig:ai-dag} by splitting each time-varying covariate into a pre-treatment node (confounder, gray) and a post-treatment node (collider, orange).
Team Skill (dashed border) remains entirely unobserved.
Solid black arrows trace the identified causal path AI Adoption $\to$ Code Generation $\to$ Dev.\ Velocity, dashed gray arrows mark back-door paths from confounders, and solid orange arrows mark edges into the post-treatment colliders.
