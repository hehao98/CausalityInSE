...
\paragraph{Comparing Estimators: Regression, PSM, and IPW}
...
The cross-sectional analysis in Section~\ref{sec:cross-sectional} revealed that the estimator is not the bottleneck---unobserved confounders are.
OLS, PSM, and IPW all converged to similar estimates (66--101\%) because none can address time-invariant unobservables like team skill.
The problem is identification, not estimation.

We now exploit the panel structure of our data---observing the \emph{same} repositories over time---to address this limitation.
Instead of comparing \emph{different} repositories (cross-section), we compare the \emph{same} repository before and after it adopts AI tools.
...
We do not need to measure or control for these factors; they cancel out in the within-unit comparison.
This is the fundamental advantage of panel data and the core idea behind design-based identification (Section~\ref{sec:primer-design}).
...
This is exactly the shift from selection-on-observables to design-based identification that Section~\ref{sec:primer-design} advocates.

However, the only way this estimate is causal is if \emph{nothing else} about the treated repositories changed between the pre and post periods, besides the treatment itself.
...
\paragraph{Interrupted Time Series.}
\label{sec:its}
...
Additionally, if the true pre-treatment dynamics are nonlinear (e.g., accelerating growth), the linear trend specification may itself be misspecified.

Unlike DiD, ITS does not have a standard formalization through the potential outcomes framework~\cite{angrist2009mostly, cunningham2021causal}.
The assumptions are stated as restrictions on the regression model rather than as properties of potential outcomes, which is one reason ITS is considered a weaker identification strategy---the assumptions are harder to state precisely and harder to test.
No amount of trend modeling can substitute for the absence of a control group.
This motivates the key design-based insight: we need a \emph{control group} to difference out common shocks.

\paragraph{Difference-in-Differences.}
\label{sec:did}

We now add a control group of 759 repositories that never adopted AI tools during the observation window.
The key idea is simple but powerful: any time-varying shock that affects both treated and control repositories equally is differenced out, \emph{even without modeling it explicitly}.
...
The double difference isolates the treatment effect.
...
\end{equation}
where $\alpha_i$ absorbs all time-invariant confounders (like ITS), $\gamma_t$ absorbs \emph{common time shocks}---the new element that the control group provides---and $\hat{\tau}$ is the DiD estimate.
...
\end{equation}
In plain language: absent treatment, treated and control repositories would have followed the same \emph{trend} in outcomes.
This does not require the two groups to have the same \emph{level} of commits---only the same trend.
Parallel trends is strictly weaker than the ITS assumption: common shocks are fine as long as they affect both groups similarly.
...
\textbf{Borusyak et al.\ (2024) imputation DiD}~\cite{borusyak2024revisiting} estimates what outcomes \emph{would have been} without treatment, then compares to what actually happened.
The procedure has three steps.
First, fit a counterfactual model on all untreated observations (never-treated repositories plus pre-treatment periods of treated repositories):
\begin{equation}
...
Second, for each treated observation, plug in the estimated coefficients to predict the counterfactual: $\hat{Y}_{it}(0) = \hat{\alpha}_i + \hat{\gamma}_t + X_{it}\hat{\beta}$.
Third, compute the ATT as the average residual $\hat{\tau} = \text{mean}(Y_{it} - \hat{Y}_{it}(0))$ across all treated observations, weighting each treated (repository, month) cell equally.
...
The estimate is $\hat{\tau} = 0.281$ ($\text{SE} = 0.060$), i.e., 32\% ($\pm$8\%).

\textbf{Callaway \& Sant'Anna (2021) DiD}~\cite{callaway2021difference} estimates separate treatment effects for each adoption cohort (group of repositories adopting in the same month), then aggregates.
...
We use no covariates, so this reduces to simple differences in means.
The overall ATT averages the group-time effects, weighting by cohort size.
This weighting differs from Borusyak's equal cell-weighting: CS gives each \emph{cohort} equal per-unit weight, while Borusyak gives each \emph{treated cell} equal weight.
With heterogeneous effects, different weights produce different aggregates.
The estimate is $\hat{\tau} = 0.416$ ($\text{SE} = 0.090$), i.e., 52\% ($\pm$14\%).

\paragraph{Why the Estimates Diverge.}
\label{sec:divergence}

The Borusyak estimate (32\%) is substantially lower than the CS estimate (52\%).
Two mechanisms explain this gap, and understanding them is pedagogically valuable because they connect directly to the DAG reasoning from Section~\ref{sec:primer-dags}.

First, \textbf{covariate attenuation}.
...
Second, \textbf{aggregation weights}.
Even without covariate attenuation, the two estimators would differ because they weight heterogeneous cohort-level effects differently---a well-known issue in the recent DiD literature~\cite{roth2023whats}.
Together, the pair of estimates brackets a plausible ATT range of 32--52\%.
The divergence is informative, not a failure: it teaches readers that methodological choices about covariate handling and aggregation have substantive consequences.

\paragraph{Event Study and Pre-Trends Assessment.}
\label{sec:event-study}

Both estimators support event-study decompositions that estimate a separate treatment effect $\tau_k$ for each month~$k$ relative to adoption.
Pre-treatment coefficients ($k < 0$) should be zero under parallel trends---they serve as a diagnostic.
...
For Borusyak, a joint Wald test pools the 12 pre-treatment coefficients ($H_0\!: \tau_{-12} = \cdots = \tau_{-1} = 0$; $p = 0.70$).
For CS, we use simultaneous confidence bands from the \texttt{did} package~\cite{callaway2021difference}, which control the family-wise error rate across all pre-treatment periods---a more conservative test than pointwise confidence intervals; 0 of 12 periods are significant under simultaneous bands.
Post-treatment coefficients show the CS estimate consistently above Borusyak, consistent with the covariate attenuation mechanism.
We note that passing a pre-trends test does not \emph{prove} parallel trends~\cite{roth2022pretest}---it only fails to reject a necessary condition.

\paragraph{Robustness.}
\label{sec:robustness}

We probe two concerns about control group composition.
...
These robustness checks probe specific threats to internal validity but cannot prove the identifying assumptions; they are best understood as ruling out specific alternative explanations.
...
Two lessons emerge from this progression.
...
OLS approximates the ATE; PSM and IPW target the ATT; all longitudinal methods target the ATT.}
...
OLS (DAG-justified) & Conditional ignorability & 0.696 & 0.090 & +101\% ($\pm$18\%) \\
PSM (1:1 NN) & Conditional ignorability & 0.574 & 0.094 & +78\% ($\pm$17\%) \\
IPW (ATT) & Conditional ignorability & 0.509 & 0.091 & +66\% ($\pm$15\%) \\
\addlinespace
...
First, the \emph{design} matters more than the estimator.
Within the cross-section, switching from OLS to PSM to IPW barely moves the estimate (66--101\%)---all three share the conditional ignorability assumption, so the estimator is not the bottleneck.
Across designs, however, the estimates change dramatically: the before/after estimate (156\%) drops to 120\% with ITS (which absorbs secular trends) and further to 32--52\% with DiD (which absorbs common shocks via a control group).
Each step trades a strong assumption for a weaker one, and the estimates shrink accordingly.
The gap between the before/after estimate and the DiD range quantifies how much of the na\"ive effect was driven by trends and selection, not treatment.
The further gap between the cross-sectional estimates (66--101\%) and the DiD range (32--52\%) quantifies the magnitude of selection bias in the cross-section---the residual confounding by time-invariant unobservables like team skill that no cross-sectional method could address.

Second, within DiD, the Borusyak--CS gap (32\% vs.\ 52\%) illustrates the mediator bias from Section~\ref{sec:primer-dags}: conditioning on post-treatment covariates that lie on the causal pathway attenuates the total effect estimate.
This is a concrete, numbers-in-hand demonstration of the DAG reasoning applied to a real analysis, reinforcing the practical importance of distinguishing confounders from mediators when selecting covariates.

The remaining threats to the DiD estimates---parallel trends violations (partially testable via event studies but never provable), anticipation effects, and SUTVA violations---are \emph{more transparent and partially testable} than the ``all confounders measured'' assumption required by cross-sectional methods.
This transparency is the central advantage of design-based identification: it makes the identification strategy open to scrutiny, so the community can constructively engage with its assumptions and iteratively develop stronger designs---which is the central thesis of this paper.
