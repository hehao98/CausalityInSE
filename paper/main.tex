...
In the first example, we retrospectively apply the toolkit to \citet{DBLP:conf/sigsoft/RayPFD14}'s landmark GitHub study and show how panel fixed effects, applied to the same data, strengthen identification by absorbing common unobserved confounders in the SE context.
In the second example, we apply the toolkit to Bogner and Merkel's cross-sectional JavaScript vs.\ TypeScript comparison, decompose the compound ``language'' treatment into the sharper ``type system adoption'' question, and redesign the study as a TypeScript migration difference-in-differences.
Both examples illustrate how the primer's pragmatic stance systematically improves prior research designs.
Persistent empirical controversies in SE reflect two orthogonal challenges: identification (can the design support a causal claim?) and measurement (are the proxies valid for the constructs of interest?).
This tutorial addresses the identification dimension; measurement validity is an equally important concern that the causal toolkit surfaces but does not itself solve.
We hope this paper will provide a methodological blueprint for future empirical SE research, illustrating how to balance rigor with practicality to generate actionable, trustworthy evidence for important empirical SE questions.
\end{abstract}
...
This exchange exposes two orthogonal methodological challenges.
The first is \emph{measurement}: Whether the outcomes being measured---bug-fix commits, code smells, defect counts---are valid proxies for the constructs of interest, and whether ``programming language'' is a well-defined treatment.
The second is \emph{identification}: Whether the research design can distinguish a genuine causal effect from confounding, even assuming the measurements are valid.
Both challenges must be addressed for empirical controversies to be resolved; this tutorial focuses on identification---the conditions under which observational data can bear a causal interpretation---while acknowledging that measurement validity is an equally important concern that the causal toolkit does not itself solve.
...
In Example~A (Section~\ref{sec:example-a}), we retrospectively apply the toolkit to \citeauthor{DBLP:conf/sigsoft/RayPFD14}'s study, showing how the cross-sectional design can be strengthened and how panel fixed effects on the same data yield more robust identification by absorbing time-invariant confounders.
\TODO{Report the empirical results of the panel FE demonstration.}
In Example~B (Section~\ref{sec:example-b}), we apply the toolkit to \citeauthor{DBLP:conf/msr/BognerM22}'s~\cite{DBLP:conf/msr/BognerM22} cross-sectional JavaScript vs.\ TypeScript comparison, decompose the compound ``language'' treatment into the specific ``type system adoption'' intervention, and redesign the study as a difference-in-differences analysis of TypeScript migration events.
