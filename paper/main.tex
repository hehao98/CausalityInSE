...
For this specific class of questions, the field's dominant approach---large-scale descriptive and/or correlational analysis of observational repository data---cannot distinguish genuine causal effects from selection biases, confounders, and other threats to causal validity.
Yet, practitioners, managers, and downstream research routinely interpret the resulting findings as causal.
...
and concludes with a pragmatic stance for SE research that synthesizes these tools into actionable guidance. %---building on prior research and practitioner knowledge to construct causal theories, framing research designs as target trials, using DAGs to make assumptions explicit, exploiting quasi-experimental variation for identification, and honestly engaging with alternative explanations.
We demonstrate the primer's diagnostic and constructive power through the programming-language and code-quality debate.
A citation analysis of 451 papers citing the landmark study by \citet{DBLP:conf/sigsoft/RayPFD14} reveals that causal interpretations outnumber hedged ones 2:1 among papers that engage with their findings.
...
\citeauthor{DBLP:journals/toplas/BergerHMVV19}, in describing the motivation for their reproduction study, directly observed that Ray~et~al.'s findings were subject to a pattern where \emph{``people misinterpreted [them] as a causal relationship''}~\cite{claburn2019register}---notably, even the article reporting this debunking uses causal language in its own headline (\emph{``lead to more buggy code''}).
