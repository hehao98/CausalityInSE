...
\TODO{ITS models the pre-treatment trend explicitly and tests for a level shift and/or slope change at the adoption event, using only treated repositories (with repository fixed effects). This improves on the na\"ive before/after comparison by separating the treatment effect from secular trends. However, without a control group, ITS cannot distinguish the treatment effect from contemporaneous shocks---e.g., a GitHub platform change, a seasonal pattern, or a field-wide shift in development practices---that affect all repositories around the same time. This limitation motivates the addition of a control group, which is the key insight of design-based identification.}

\paragraph{Difference-in-Differences.}
\label{sec:did}

\TODO{The design-based leap: Adding a control group of repositories that did not adopt AI tools. Build DiD intuition in two steps. First, the classic 2$\times$2 diagram (treated vs.\ control $\times$ pre vs.\ post): The double difference $({\bar Y}^{\text{post}}_{\text{treated}} - {\bar Y}^{\text{pre}}_{\text{treated}}) - ({\bar Y}^{\text{post}}_{\text{control}} - {\bar Y}^{\text{pre}}_{\text{control}})$ subtracts out the common time trend using the control group---exactly what ITS could not do. Second, the TWFE regression specification $Y_{it} = \alpha_i + \gamma_t + \tau D_{it} + \varepsilon_{it}$, where $\alpha_i$ absorbs time-invariant confounders, $\gamma_t$ absorbs common shocks, and $\hat\tau$ is the DiD estimate. Note immediately that in our staggered adoption setting (different repos adopt at different times), the classic TWFE estimator uses already-treated units as implicit controls for later-treated units, producing biased estimates when treatment effects are heterogeneous~\cite{goodman2021difference, dechaisemartin2020two, sun2021estimating}. Refer readers to \citet{roth2023whats} for the full landscape of recent DiD methodology.}

\TODO{Present two complementary modern DiD estimators. (1) \textbf{Borusyak et al.\ (2024) imputation DiD} with time-varying covariates: Fit a model for $Y(0)$ using only untreated observations (never-treated + not-yet-treated), impute the counterfactual for each treated $(i,t)$ cell, and average the residuals. This avoids the negative-weighting problem of TWFE. Include time-varying covariates (log contributors, stars, issues, forks, releases) in the first-stage imputation model. (2) \textbf{Callaway \& Sant'Anna (2021) outcome-regression DiD} without covariates: estimate group-time ATTs using only unit and time effects, then aggregate.}

\TODO{The comparison between the two estimators is pedagogically valuable and makes for the central interpretive discussion of this subsection. The Borusyak estimate (ATT $\approx$ 0.28) is substantially lower than the CS estimate (ATT $\approx$ 0.42). Two mechanisms explain this gap. First, \textbf{covariate attenuation}: Borusyak's first-stage model includes time-varying covariates and plugs in their \emph{post-treatment} values when imputing $Y(0)$ for treated cells. If these covariates are post-treatment mediators (e.g., AI adoption increases contributors/stars, which in turn increase commits), the imputed counterfactual is inflated, shrinking the estimated treatment effect. The CS specification omits covariates entirely and is unaffected. Second, \textbf{aggregation weights}: Borusyak weights each treated $(i,t)$ cell equally, while CS averages within cohort-time cells then across cohorts weighted by group size. With heterogeneous effects across cohorts, different weighting produces different aggregates. Together the two estimates bracket a plausible range for the ATT.}

\TODO{Present a combined event-study plot (Figure~\ref{fig:event-study}) overlaying both estimators. Pre-treatment coefficients near zero for both support the parallel trends assumption; assess pre-trends formally (Borusyak: joint Wald test, $p = 0.70$; CS: simultaneous confidence bands, 0/12 periods significant). Post-treatment coefficients show the dynamic treatment effect with CS consistently above Borusyak, consistent with the covariate attenuation mechanism.}

\TODO{Robustness: Assess sensitivity of the Borusyak ATT to control group composition: compare estimates across the full sample and a trimmed sample (dropping structurally inactive repos with $<$ 10 total commits). Stability of the ATT across these samples addresses concerns about the parallel trends assumption being driven by a specific subset of controls.}

\paragraph{Longitudinal Takeaway.}
\label{sec:longitudinal-takeaway}

\TODO{Compare all longitudinal estimates (before/after, ITS, Borusyak DiD, CS DiD) side by side. The progression illustrates the core message: the \emph{design} matters more than the estimator. Before/after and ITS, which lack a control group, produce estimates confounded by secular trends. DiD, by introducing a control group and exploiting staggered adoption as quasi-random variation, absorbs both time-invariant confounders (via unit FE) and common shocks (via the control group counterfactual). The gap between the Borusyak and CS estimates illustrates a second lesson: when time-varying covariates are potential post-treatment mediators, conditioning on them attenuates the estimate. The pair of DiD estimates brackets a plausible ATT range (0.28--0.42). The remaining threats---anticipation effects, parallel trends violations---are more transparent and partially testable than the all-confounders-measured assumption required by cross-sectional methods. Contrast the DiD range with the cross-sectional estimates from Section~\ref{sec:cross-sectional}: the difference quantifies the magnitude of selection bias in the cross-section.}
