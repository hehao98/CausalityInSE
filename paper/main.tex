...
A na\"ive analyst might interpret the raw gap in monthly commits as evidence that AI tools boost productivity, but Section~\ref{sec:primer-naive} explained why such comparisons conflate any genuine effect with selection bias.
...
All count variables are log-transformed with $\log(1+x)$.}
...
\item[a] We make a simplying assumption that organization ownership and primary language is pre-treatment with the intuition that both should be very rare for mature and established repositories in our observed time window (2024 - 2026). Technically, both can change over time (repositories may transfer between owners, and the primary language label may shift as the codebase evolves).
...
A researcher following standard MSR practice might stop here and conclude that \emph{there is suggestive correlational evidence that AI adoption is associated with \textbf{157.5\%} (i.e., $e^{0.946} - 1$) higher development velocity, holding all observables constant}.
...
To answer this question, we apply the DAG framework from Section~\ref{sec:primer-dags}.
Recall that Figure~\ref{fig:ai-dag} presented a plausible DAG for the AI tools--velocity question, with a footnote noting that the role of Stars is temporally ambiguous.
We now resolve this ambiguity by constructing the proper \emph{temporal DAG} that time-indexes each variable.

Consider Stars as an example.
In the true data-generating process, stars accumulate over time, so the actual causal DAG has time-indexed nodes:
\begin{itemize}[leftmargin=15pt]
    \item $\text{Stars}_{t-1}$ (\emph{before} AI adoption) is a \textbf{confounder}: Popular projects attract AI-savvy contributors ($\text{Stars}_{t-1} \to \text{Adoption}_t$) \emph{and} have more active contributor bases ($\text{Stars}_{t-1} \to \text{Commits}_t$).
    \item $\text{Stars}_{t+1}$ (\emph{after} AI adoption) is a \textbf{collider}: Both AI adoption and high commit activity increase a project's visibility ($\text{Adoption}_t \to \text{Stars}_{t+1} \leftarrow \text{Commits}_t$).
\end{itemize}
In this temporal DAG, the adjustment set is unambiguous: Condition on $\text{Stars}_{t-1}$, never on $\text{Stars}_{t+1}$.
...
\node[var] (org) at (2.8, 3.5) {Organization};
\node[unobs] (skill) at (5.6, 3.5) {Team Skill};
\node[var] (maturity) at (8.4, 3.5) {Proj.\ Maturity};

% Pre-treatment time-varying confounders (second row)
\node[var] (pop_pre) at (0, 2) {Popularity$_{t\text{-}1}$};
\node[var] (demand_pre) at (2.8, 2) {Demand$_{t\text{-}1}$};
\node[var] (activity_pre) at (5.6, 2) {Activity$_{t\text{-}1}$};
\node[var] (team_pre) at (8.4, 2) {Team Size$_{t\text{-}1}$};

% Main causal chain (middle)
\node[var] (treat) at (1.4, 0.5) {AI Adoption};
\node[var] (codegen) at (4.6, 0.3) {Code Gen.};
\node[var] (workflow) at (4.6, 0.8) {Workflow};
\node[var] (outcome) at (7.7, 0.5) {Dev.\ Velocity};

% Post-treatment colliders (bottom row)
\node[var] (pop_post) at (0, -1) {Popularity$_{t\text{+}1}$};
\node[var] (demand_post) at (2.8, -1) {Demand$_{t\text{+}1}$};
\node[var] (activity_post) at (5.6, -1) {Activity$_{t\text{+}1}$};
\node[var] (team_post) at (8.4, -1) {Team Size$_{t\text{+}1}$};

% Structural confounders -> treatment and outcome
\draw[arr] (domain) -- (treat);
\draw[arr] (domain.south) to[bend right=10] (outcome.north);
\draw[arr] (org) -- (treat);
...
\end{tikzpicture}
\caption{Temporal DAG for the AI coding tools and development velocity problem, resolving the temporal ambiguity noted in Figure~\ref{fig:ai-dag}.
...
A typical MSR study queries the GitHub API once at the time of the study, obtaining a single \emph{snapshot} value for each covariate.
This snapshot \emph{collapses} the time-indexed nodes into a single atemporal variable.
For Stars, the snapshot merges $\text{Stars}_{t-1}$ (confounder) and $\text{Stars}_{t+1}$ (collider) into a single ``Stars'' node.
In the resulting graph, Stars has both incoming and outgoing arrows to both treatment and outcome---the graph is \emph{cyclic}, not a DAG, and no adjustment formula applies.

This temporal collapse is not specific to Stars: Every time-varying covariate in our dataset---PRs, issues, forks, repository size, releases, CI/CD presence, contributor count---suffers from the same problem.
The snapshot value includes both pre-treatment variation (which should be conditioned on) and post-treatment variation (which should not), and the two are inextricably mixed.
...
Table~\ref{tab:covariate-classification} classifies the available covariates by temporal status.
Only three covariates are \emph{structurally pre-treatment}: Repository age (determined at creation), primary programming language (determined at creation), and organization ownership (determined at creation).
These cannot be affected by AI adoption under any plausible timing and are therefore defensible controls in the cross-sectional snapshot.
All remaining covariates are \emph{temporally ambiguous}: Their snapshot values contain both pre-treatment and post-treatment variation, and conditioning on them may introduce collider bias, block causal paths, or both.
...
But the three structurally pre-treatment covariates available in the cross-sectional snapshot (repo age, language, organization) are weak proxies for the confounders that drive selection into AI adoption.
The temporal DAG identifies the full adjustment set---all pre-treatment values of time-varying covariates---but the cross-sectional snapshot destroys them.

Fortunately, our panel dataset (Section~\ref{sec:study-setting}) provides monthly observations for each repository.
For treated repositories, we know the exact treatment month; for control repositories, all months are pre-treatment.
We aggregate only pre-treatment months to recover genuinely pre-treatment values of the time-varying covariates: pre-treatment stars, issues, forks, PRs merged, releases, average active contributors, and average monthly commits.
This restores the clean temporal structure that the snapshot destroyed and allows us to condition on the full DAG-justified adjustment set---minus Team Skill, which remains entirely unobserved.
...
The bivariate coefficient is $2.314$ ($R^2 = 0.21$), reflecting raw selection.
The kitchen-sink coefficient attenuates to $0.946$ ($R^2 = 0.59$) after adding all available controls, but its interpretation is incoherent: The collapsed cross-sectional graph is not a DAG, so no adjustment formula tells us whether we are removing confounding bias, blocking causal paths through mediators, or opening spurious paths through colliders.
The DAG-justified specification further attenuates to $0.696$ ($R^2 = 0.83$), conditioning only on covariates that the temporal DAG identifies as pre-treatment confounders, each computed from panel data before the treatment month.
This is the only specification with a principled justification from the back-door criterion (Section~\ref{sec:primer-dags}).

Pre-treatment Activity is the most important addition: It captures baseline development velocity, the strongest predictor of both AI adoption (high-velocity projects have incentive to adopt) and future velocity (persistence).
Including it effectively asks: \emph{Conditional on repositories having similar pre-treatment activity, popularity, demand, and team size, do AI-adopting repositories subsequently exhibit higher commit rates?}

\begin{table}[t]
\centering
\caption{OLS estimates of the AI adoption effect on $\log(\text{monthly commits})$.
...
All standard errors are heteroscedasticity-robust (HC1).}
...
How robust is the DAG-justified estimate to this unobserved confounding?

We apply the Oster~\cite{oster2019unobservable} proportional selection method (Section~\ref{sec:primer-po}), which asks: Given how much the treatment coefficient changed when we added the observable pre-treatment controls, how strong would selection on unobservables need to be---relative to that same selection on observables---to drive the coefficient all the way to zero?
The key quantity is $\delta^*$.
The conventional threshold is $\delta^* \geq 1$: If unobservables would need to be \emph{at least as strong as} all the observables combined to eliminate the effect, the result is considered robust.

Moving from the bivariate to the DAG-justified specification, $\delta^* = 0.12$---far below the robustness threshold of~1.
...
\end{enumerate}
In plain language: Given how much the coefficient already dropped and how much variance the observables already explain, even a relatively weak unobserved confounder could eliminate the remaining effect.
The most plausible candidate---developer skill and team culture---is precisely the confounder that no GitHub API data can address, and it is plausibly strong enough to account for the residual association.

\paragraph{Estimator Convergence: OLS, PSM, and IPW.}
\label{sec:estimator-convergence}

A natural response to the fragility of the OLS estimate might be to try a ``better'' estimator.
Propensity score matching (PSM) and inverse probability weighting (IPW) are increasingly popular in SE research, sometimes presented as though they automatically confer causal credibility.
If the problem were model misspecification---nonlinearity, poor functional form, or lack of overlap---these methods might indeed help, because they relax the linearity assumptions of OLS\@.
But if the problem is unobserved confounding, all three estimators will converge on the same biased answer, because they share the same identifying assumption: conditional ignorability given the observed pre-treatment covariates.

Table~\ref{tab:three-estimators} confirms this prediction.
Using the same DAG-justified pre-treatment covariates, OLS ($\hat{\tau} = 0.696$), 1:1 nearest-neighbor propensity score matching ($\hat{\tau} = 0.574$), and ATT-targeted inverse probability weighting ($\hat{\tau} = 0.509$) yield point estimates that are all positive, statistically significant ($p < 0.001$), and have overlapping confidence intervals.
The three estimators target slightly different estimands---OLS approximates the ATE while PSM and IPW estimate the ATT---and operate on different effective samples (PSM retains 456 matched observations from the 999 available), which accounts for the modest variation in point estimates.
But the substantive conclusion is identical: \emph{Switching from regression to matching to weighting changes nothing because all three share the same identifying assumption, and that assumption is violated by the unobserved confounders that no GitHub API data can capture.}
Using PSM or IPW does not automatically make a study ``causal inference.''

\begin{table}[t]
\centering
\caption{Three estimators using the same DAG-justified pre-treatment covariates.
All three yield positive, significant estimates with overlapping confidence intervals, confirming that the estimator is not the bottleneck---the identifying assumption is.
All standard errors are heteroscedasticity-robust (HC1).}
...
\midrule
OLS (DAG-justified) & 0.696 & 0.090 & ${<}0.001$ & 999 \\
PSM (1:1 nearest neighbor) & 0.574 & 0.094 & ${<}0.001$ & 456 \\
IPW (ATT) & 0.509 & 0.091 & ${<}0.001$ & 999 \\
\bottomrule
\end{tabular}
\end{table}
...
The cross-sectional analysis demonstrates a general problem for MSR studies that seek causal conclusions from snapshot data.
Cross-sectional designs suffer from \emph{temporal collapse}: The measurement design destroys the temporal ordering that DAGs require, making principled covariate selection impossible for time-varying covariates.
Even when panel data are available to recover pre-treatment covariate values, the resulting estimates remain fragile to unobserved confounding by unmeasurable factors such as developer skill and team culture.
And switching to a more sophisticated selection-on-observables estimator (PSM, IPW) does not help, because the bottleneck is the identifying assumption---conditional ignorability given observables---not the choice of estimator.
Only within-repository longitudinal variation---comparing the \emph{same} project before and after adoption, thereby absorbing all time-invariant confounders---can make progress on identification.
