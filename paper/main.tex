...
A natural question arises: can we sharpen the estimate by conditioning on time-varying repository characteristics---contributor counts, stars, issues---to make the parallel trends assumption more plausible?

\textbf{Borusyak et al.\ (2024) imputation DiD}~\cite{borusyak2024revisiting} does exactly this.
Rather than differencing group means, it estimates what outcomes \emph{would have been} without treatment, using a covariate-adjusted counterfactual model, then compares to what actually happened.
...
Surprisingly, adding covariates \emph{lowers} the estimate rather than sharpening it.
CS without covariates gives 52\%; Borusyak with covariates gives 32\%.
This counterintuitive result motivates closer inspection of what the covariates are doing.
Note also that the two estimators weight heterogeneous cohort-level effects differently: CS gives each \emph{cohort} equal per-unit weight, while Borusyak gives each \emph{treated cell} equal weight.
...
Within DiD, the CS--Borusyak comparison (52\% vs.\ 32\%) illustrates a subtle but important lesson: conditioning on post-treatment covariates that lie on the causal pathway attenuates the total effect---a concrete instance of the mediator bias from Section~\ref{sec:primer-dags}.
...
All OLS specifications approximate the ATE; PSM and IPW target the ATT; all longitudinal methods target the ATT.}
...
Yet within the DAG-justified cross-section, switching from OLS to PSM to IPW barely moves the estimate (66--101\%): all three share the conditional ignorability assumption, so the estimator is not the bottleneck.
Across designs, the estimates change dramatically: the before/after estimate (156\%) drops to 120\% with ITS (which absorbs secular trends) and further to 32--52\% with DiD (which absorbs common shocks via a control group).
...
Second, within DiD, the CS--Borusyak gap (52\% vs.\ 32\%) illustrates the mediator bias from Section~\ref{sec:primer-dags}: conditioning on post-treatment covariates that lie on the causal pathway attenuates the total effect estimate.
