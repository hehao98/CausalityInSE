...
We demonstrate the primer's diagnostic and constructive power through the programming-language and code-quality debate: A citation analysis of 451 papers citing the landmark study by \citet{DBLP:conf/sigsoft/RayPFD14} reveals that causal interpretations outnumber hedged ones 2:1 among papers that engage with the finding, quantifying the downstream misinterpretation problem.
We then apply the pragmatic stance to two worked examples from this debate.
...
Third, we demonstrate the primer's diagnostic and constructive power through the programming language and code quality debate (Section~\ref{sec:pl-debate}): A literature synthesis traces the debate's three-decade trajectory (Section~\ref{sec:pl-lit-synthesis}), and a citation analysis of 451 papers citing \citeauthor{DBLP:conf/sigsoft/RayPFD14}'s study quantifies how correlational findings are systematically reinterpreted as causal claims downstream (Section~\ref{sec:pl-misinterpretation}).
We then apply the pragmatic stance to two worked examples.
...
Section~\ref{sec:pl-debate} presents the programming language and code quality debate as a unifying case study: a literature synthesis (Section~\ref{sec:pl-lit-synthesis}), an analysis of downstream causal misinterpretation (Section~\ref{sec:pl-misinterpretation}), and the two worked examples (Sections~\ref{sec:example-a} and~\ref{sec:example-b}).
...
First, prior quantitative studies provide the raw material for proposing causal structures: An observed association between programming language adoption and defect rates does not itself establish causation, but it motivates a DAG edge that can then be subjected to rigorous identification.
...
\paragraph{Promises and Perils of Causal Inference in SE Research.}
...
Second, SE treatments are often \emph{compound}: For example, ``adopting TypeScript'' bundles a type system, a compiler toolchain, a migration effort, and a self-selected team; in this case, the researcher should properly interpret the effect as such rather than from a single oversimplified treatment (e.g., ``just use TypeScript to reduce defects'').
...
Finally, SE outcomes are typically \emph{noisy proxies} for the constructs of interest: Bug-fix commits proxy for defect proneness, lines of code proxy for complexity, and star counts proxy for popularity---each introducing measurement error that may attenuate effect estimates and complicate interpretation.
...
Importantly, each peril has a principled response within the toolkit so that SE researchers can engage with the specific threats in their settings.
...
We choose the programming language and code quality debate as this tutorial's unifying case study because it illustrates, within a single research thread, the full arc of methodological challenges that causal inference addresses: contested observational findings, widespread downstream misinterpretation, and an impasse that better statistics alone cannot resolve.
We do not aim to provide the ultimate empirical evidence to resolve this debate (which would be far beyond the scope of this tutorial), but to retrospectively illustrate how the causal inference toolkit can be used to diagnose and improve prior study designs.
This section first synthesizes the debate's three-decade trajectory (Section~\ref{sec:pl-lit-synthesis}), then documents how correlational findings are systematically reinterpreted as causal claims (Section~\ref{sec:pl-misinterpretation}), and finally applies the pragmatic stance to two studies from this debate---diagnosing their identification limitations and showing how the toolkit generates improved designs (Sections~\ref{sec:example-a}--\ref{sec:example-b}).
...
Yet the downstream reception of this literature tells a different story.
We provide two streams of evidence---a quantitative citation analysis and a survey of practitioner discourse---documenting a systematic pattern in which correlational findings are reinterpreted as causal claims.

\paragraph{Citation Analysis.}
We retrieved all 451 papers citing \citet{DBLP:conf/sigsoft/RayPFD14} or \citeauthor{DBLP:journals/cacm/RayPDF17}~\cite{DBLP:journals/cacm/RayPDF17} from the Semantic Scholar API and manually classified the citation context of each paper with available full-text snippets ($n = 339$) into four categories: \emph{causal} (the citing paper treats or implies the finding as causal), \emph{hedged} (accurately describes the finding as an association or correlation), \emph{neutral} (cites for context, dataset, or methodology without interpreting the PL--quality finding), and \emph{critical} (questions the methodology or findings).
Among the 339 papers with citation contexts, 24\% use causal framing---language such as ``the effect of programming languages on software quality,'' ``statically typed languages are less error-prone,'' or ``language design impacts code quality''---while only 12\% use appropriately hedged language and 2\% engage critically with the study's identification limitations.
...
The misinterpretation is not the result of a few careless citations; it is the \emph{modal} interpretation among papers that engage with the finding substantively.
...
\citeauthor{DBLP:journals/toplas/BergerHMVV19}, in describing the motivation for their reproduction study, directly observed that Ray~et~al.'s findings were subject to a pattern where ``people misinterpreted [them] as a causal relationship.''\footnote{%
  T.\ Claburn, ``Boffins debunk study claiming certain languages lead to more buggy code,'' \emph{The Register}, January 30, 2019.
  Even the debunking article's headline uses causal language (``lead to more buggy code'').}
Blog posts cite the study to rank languages by ``bug-proneness'' as if it were an intrinsic language property,\footnote{%
  E.\ Huang, ``5 Programming Language That Produce Code Least Prone to Bugs,'' personal blog, March 2, 2021.
  Uses causal verbs (``produce,'' ``eliminate'') while citing Ray~et~al.\ as empirical backing.}
and within days of the original publication, practitioners on Hacker News interpreted regression coefficients as switchable causal effects---e.g., ``A Haskell project can expect to see 63\% of the bug fixes that a C++ project would see.
I don't call a 36\% drop in bugs `small'\,''---treating the coefficient as if switching from C++ to Haskell \emph{would} directly produce fewer bugs, holding all else constant.\footnote{%
  Hacker News discussion thread \#8558740, November 4--5, 2014.}
