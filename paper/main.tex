...
\begin{abstract}
Empirical software engineering (SE) research frequently investigates questions about the causal effects of interventions---does adopting a new language reduce defects? does code review improve quality? does CI/CD adoption increase release reliability?
For this specific class of questions, the field's dominant approach---large-scale analysis of observational repository data with regression controls---cannot distinguish genuine causal effects from selection biases and confounding, yet practitioners, managers, and downstream research routinely interpret the resulting findings as causal.
...
Yet a specific class of empirical questions---those about the \emph{causal effect} of interventions on outcomes---remains difficult to answer convincingly with the field's dominant methodological tools.
When practitioners ask whether adopting code review reduces defects~\cite{DBLP:journals/ese/McIntoshKAH16}, whether CI/CD improves release quality~\cite{DBLP:conf/kbse/HiltonTHMD16}, or whether a typed language decreases bug rates~\cite{DBLP:conf/sigsoft/RayPFD14}, they are asking \emph{causal} questions that demand more than correlational evidence.
...
The problem is compounded downstream: Even when authors carefully qualify findings as associational, practitioners, managers, and downstream papers often interpret them causally---a pattern well documented in health research, where \citet{haber2022causal} found that 52\% of abstracts imply causality despite ostensibly associational language, and where \citet{hernan2018cword} argued that avoiding causal language does not prevent causal interpretation but merely obscures the underlying assumptions.
...
The 2021 Nobel Prize in Economic Sciences, awarded to Card, Angrist, and Imbens~\cite{nobelprize2021economics}, recognized these contributions as among the most consequential methodological advances in modern social science.
...
Our tutorial treats both traditions as essential for SE research: Design-based identification provides credible effect estimates for SE interventions while graphical causal models help decompose the mechanisms through which the intervention affects the outcome (Section~\ref{sec:primer}).
For a detailed historical account of these intellectual traditions and parallels with methodological reforms in psychology and epidemiology, see Appendix~\ref{sec:appendix-history}.
...
While the DAG is less central to identification for design-based methods, it remains valuable for reasoning about which confounders the design absorbs, what remaining threat existings, and more importantly, what is the actual \emph{mechanism} behind the observed cause and effect.
A genuine and rigorous reasoning on causal mechanisms often reveal important design considerations and practical implications, as we will show in the second worked example on the effect of TypeScript (Section~\ref{sec:example-b}). 
...
Section~\ref{sec:guide} operationalizes this pragmatic stance as a practical guide, and Appendix~\ref{sec:appendix-standard} provides a concrete empirical standard for structured assessment of research works.


\section{A Guide for Conducting Credible Causal Inquiries in Software Engineering}
\label{sec:guide}
...
This appendix provides a detailed account of the intellectual traditions that produced the causal inference toolkit introduced in Section~\ref{sec:bg-causal-history}, followed by a summary of methodological reforms in psychology and epidemiology that address problems analogous to those we identify in SE\@.
...
Statisticians laid the \emph{experimental foundations}.
...
Econometricians drove the \emph{credibility revolution}, shifting the basis of causal claims from statistical modeling to research design.
...
Psychology and health research have confronted a problem analogous to the one we identify in SE---a disconnect between causal ambition and methodological practice---and their experience reveals both consequences and remedies relevant to the SE community.
...
\citet{haber2022causal} provided large-scale empirical evidence that this norm fails on its own terms---among 1,170 articles from 18 high-profile health journals, 52\% of abstracts implied moderate or strong causality despite ostensibly associational language.

In response, methodological reformers applied tools from the causal inference traditions described above.
...
Institutionally, \citet{lederer2019control} issued editor-endorsed guidance requiring authors to specify causal questions and justify covariate selection.
...
\subsection{A complete causal model of a software system might require thousands of variables. Does that mean causal inference is hopeless without a complete theory?}
...
Design-based methods (Section~\ref{sec:primer-design}) sidestep the completeness problem entirely.
...
A model can achieve near-perfect $R^2$ while producing severely biased causal estimates.
...
\citeauthor{oster2019unobservable}'s~\cite{oster2019unobservable} coefficient stability test exploits the \emph{pattern} of $R^2$ changes as controls are added to assess vulnerability to omitted variable bias.
But this test uses the $R^2$ trajectory as a diagnostic signal, not $R^2$ itself as a guarantee.
The lesson: $R^2$ tells you how well you predict; it tells you nothing about whether your treatment coefficient has a causal interpretation.

\subsection{I have a multivariate regression with many covariates. Doesn't controlling for more variables always reduce bias?}

No---and in many cases, adding variables \emph{increases} bias.
This counterintuitive result follows directly from the causal structure of the data-generating process.

Adding a variable can introduce three distinct problems, each formalized in the DAG framework (Section~\ref{sec:primer-dags}).
First, \emph{mediator bias}: If a variable lies on the causal path from treatment to outcome (e.g., controlling for tooling quality when studying the effect of TypeScript adoption), conditioning on it blocks the genuine effect.
Second, \emph{collider bias}: If a variable is a common effect of treatment and outcome (e.g., project popularity influenced by both language choice and code quality), conditioning on it opens a spurious path that creates an association where none exists in the full population.
Third, \emph{overcontrol bias}: If a variable is a descendant of the treatment (e.g., test coverage that changes \emph{because} a team adopted a new practice), conditioning on it partially blocks the causal pathway.

``Throwing in everything available'' is not a conservative strategy---it is a gamble whose expected payoff depends on the causal structure, which regression alone cannot reveal~\cite{wysocki2022statistical}.
...
\citet{westreich2013table2} documented the pervasiveness of this error (the ``Table~2 fallacy'') in epidemiology; the same pattern is widespread in empirical SE research.
...
A researcher using DiD does not \emph{need} a complete DAG, but one who draws even a partial DAG reasons more clearly about what their design does and does not address.
...
The dominant SE practice---fitting a regression, reporting the coefficient on the treatment variable, and interpreting it in causal language in the discussion---is exactly the pattern that the ``Table~2 fallacy''~\cite{westreich2013table2} describes.
...
Several common situations break or weaken the experimental guarantee, and all are prevalent in SE research.
...
\emph{Non-randomizable attributes.}
Many SE-relevant ``treatments'' cannot be randomly assigned: developer experience, gender, educational background, programming language fluency.
...
\emph{SUTVA violations.}
If treated and control developers interact---on the same team, same codebase, same communication channels---the treatment can spill over, violating the stable unit treatment value assumption (Section~\ref{sec:primer-po}) that the experimental design requires.

\emph{External validity.}
A lab experiment with students on toy tasks may have clean identification but tell us nothing about production software at scale.
The target trial framework helps evaluate this gap between the study's actual scope and the research question's intended scope.

The broader lesson: Causal reasoning is not a tax imposed on observational studies that experiments get to avoid.
...
The guide in Section~\ref{sec:guide} walks the researcher through this assessment, and the empirical standard in Appendix~\ref{sec:appendix-standard} provides verifiable criteria for reviewers.
...
The guide (Section~\ref{sec:guide}) exists precisely to walk SE researchers through the reasoning process, and the empirical standard (Appendix~\ref{sec:appendix-standard}) provides verifiable criteria for reviewers.
...
Read Section~\ref{sec:primer} for the conceptual foundations, then follow the guide in Section~\ref{sec:guide}.
