...
It is technically correct, but, as typically deployed, not particularly useful. 
...
For example, when a study reports that teams using certain tools, practices, or processes (e.g., AI coding assistants, CI/CD, code review) ship features faster than teams that do not, readers (especially practitioners deciding whether to invest in such tools) naturally want to read this as evidence that adoption causes the improvement.\footnote{
...
To this end, we provide a tutorial on causal inference for empirical SE, drawing on three intellectual traditions that have proven broadly useful across different fields of empirical sciences.
First, the potential outcomes framework~\cite{neyman1923application, rubin1974estimating} lay the mathematical foundations for defining what causal quantity we want to estimate (i.e., the \emph{target estimand}) and under which formal conditions the causal quantity can be estimated from a specific statistical model and study design (i.e., the \emph{the identifying assumptions}).
Second, the directed acyclic graphs (DAGs) from \citet{pearl2009causality} provide a visual language for articulating the researcher's belief on what might be an underlying causal theory that can approximate the true data-generating process, which is particularly useful for reasoning about the \emph{causal mechanisms} through which the intervention affects the outcome, and for a family of methods (e.g., regression, matching, invert probability weighting), it is the only way to argue that the identifying assumption is plausible.

directed acyclic graphs (DAGs), a graphical notation for making causal assumptions visible and amenable to scrutiny, the potential outcomes framework, a vocabulary for stating precisely what effect is being estimated, and design-based identification, a framework for deriving the assumptions under which certain type of observational evidence can support a causal interpretation. We introduce all three at a level accessible to researchers without formal training in causal inference, and we illustrate their use through a running example: estimating the impact of AI coding assistant adoption on development productivity.

We begin with the most direct approach, comparing measured outcomes between a sample of teams that adopted AI tools and teams that did not, and use a DAG to surface the assumptions this comparison requires. We then show how plausible violations of those assumptions (for example, that early-adopting teams may already differ in ways that affect productivity) motivate progressively stronger designs: longitudinal comparisons, difference-in-differences, and a staggered difference-in-differences approach that exploits variation in adoption timing. At each stage, the DAG makes visible which sources of confounding are addressed by the design, which remain, and what the researcher must assume for the analysis to yield a credible causal estimate. Each design involves a different constellation of assumptions and trade-offs, and reasoning about these explicitly is more productive than retreating to the blanket disclaimer that correlation does not imply causation.

The credibility of a causal argument rests on more than the statistical method. A researcher with deep domain knowledge about the mechanisms through which AI tools might affect productivity, about the organizational factors that drive adoption decisions, can construct a more compelling argument from observational data than one who applies a sophisticated estimator without such understanding. DAGs provide a way to encode and communicate this domain knowledge, making it available for scrutiny and debate, though they are a tool for transparency, not a substitute for expertise.

Our perspective builds on and complements recent calls for greater transparency in empirical software engineering methodology. Robillard et al. [2] argued that threats-to-validity sections should evolve into explicit discussions of study design trade-offs, with stated rationale and implications. We extend this idea to the specific domain of causal reasoning, focusing on the assumptions that connect a study's design to the inferential claims it can support. Rohrer [1] demonstrated how the same causal inference frameworks used here can help psychologists (another field where observational studies are common and causal questions are central) move from implicit causal reasoning to explicit, assumption-transparent arguments.

The contribution of this paper is methodological and pedagogical. We do not introduce new statistical methods; we aim to enrich how the software engineering community reasons and communicates about the inferential reach of empirical studies, moving from a binary framing ("experiment = causal, observational = not") toward a more graduated understanding in which the strength of a causal argument depends on the plausibility of its assumptions. We provide concrete guidance on identifying, representing, and reporting those assumptions, with the hope that this leads to research in which the causal reasoning is laid out plainly, so the community can engage with it constructively.
...
(2)~whether it investigates (or implies to be motivated by) an intervention-outcome research question, and (3) whether it claims to employ a causal inference method.
...
We adopt deterministic decoding and an explicit classification schema to maximize reproducibility; all prompts, scripts, and classified data are available in the replication package.

\paragraph{The Causal Ambition--Methodology Gap.}
Of the 5,341 papers, 2,136 (40\%) are empirical SE studies; 621 (29\%) explicitly ask or are implicitly motivated by intervention-outcome research questions (RQs); but only 99 (1.9\%) papers claim to employ any recognizable causal inference method (Table~\ref{tab:taxonomy}).
The vast majority of the 621 papers investigate their proposed RQs through interpreting correlational, descriptive, or qualitative evidence.
Figure~\ref{fig:causal-trends} plots this gap over time: While the absolute number of papers asking intervention-outcome RQs has grown steadily---from 23 in 2015 to 131 in 2025---the number claiming to employ causal inference methods has grown far more slowly, from 3 to 14 over the same period.
The ratio of intervention-outcome RQs to causal inference methods has remained roughly 7-to-1 throughout the decade, indicating that the gap is structural rather than transitional.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{causal_trends.pdf}
\caption{Publication trends at ICSE, FSE, and ASE (2015--2025).
The top two lines show total publications and empirical SE papers; the bottom two show papers asking intervention-outcome research questions (RQs) versus papers employing causal inference methods.
...
Causal fault localization---applying causal testing, causal contribution scores, or causal debugging to identify root causes of failures---accounts for 13 papers, representing a growing research thread that applies causal reasoning to software analysis rather than to empirical research methodology.
...
Two patterns emerge from this analysis.
