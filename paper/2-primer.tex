...
Because a DAG encodes the researcher's substantive theory of the data-generating process---not knowledge extracted from the data itself---each node and edge should rest on a specific source of prior evidence: published quantitative studies, controlled experiments, qualitative investigations, or, where these are absent, explicit domain knowledge that can be challenged.
To illustrate this discipline, we construct a pedagogical DAG for the AI coding tools--velocity question (Figure~\ref{fig:ai-dag}) by anchoring each node in the relevant literature, and we deliberately keep the figure to one mediator and one collider so that the construction exercise is transparent end-to-end.
\emph{The treatment $\to$ mediator $\to$ outcome path.}
A growing body of randomized experiments establishes that AI coding tools accelerate per-task code production: \citet{cui2025effects} pool three firm-level field trials ($N = 4{,}867$ developers across Microsoft, Accenture, and a Fortune-100 company) and report a $+26\%$ increase in completed tasks; \citet{peng2023impact} find a $55.8\%$ reduction in completion time on a controlled JavaScript task; and \citet{DBLP:conf/icse-seip/ParadisGMNMMZFC25} report an $\approx 21\%$ time reduction on an enterprise task at Google.
This convergent experimental evidence motivates \emph{Code Generation} as the load-bearing mediator on the AI Tool Adoption $\to$ Dev.\ Velocity path.
\emph{The collider.}
\citet{DBLP:conf/icse/FangLHV22} use a difference-in-differences design to show that visibility shocks (specifically, social-media promotion) causally increase a GitHub project's star count and contributor inflow; a symmetric argument applies to development activity, as actively-developed projects---like AI-adopting ones---are differentially likely to attract attention in 2024--2026 commentary on developer-tool trends.
Popularity is therefore a \emph{collider} on the AI Tool Adoption $\to$ Popularity $\leftarrow$ Dev.\ Velocity path, and conditioning on it would open a spurious association rather than close one.
\emph{The confounders.}
We include three confounders that domain knowledge identifies as likely to drive both adoption and velocity: \emph{Project Maturity}, \emph{Domain} (primary programming language), and \emph{Team Skill}.
The first two are structurally pre-treatment characteristics that adopters and non-adopters differ on substantially---we will see in Section~\ref{sec:naive-comparison} (Table~\ref{tab:descriptive}) that adopters are younger (SMD~$=-0.37$) and heavily skewed toward TypeScript (SMD~$=+0.50$), consistent with the practitioner expectation that AI tools work unevenly across language ecosystems and that younger projects are typical early adopters of new developer tools.
\emph{Team Skill}---encompassing developer expertise, team culture, and organizational practice---is the canonical unobserved confounder in this setting, since GitHub repository data captures no proxy for it; Figure~\ref{fig:ai-dag} marks Team Skill with a dashed border to make this unobservability visually explicit, and Section~\ref{sec:cross-sectional-sensitivity} shows that the cross-sectional estimate is highly sensitive to its plausible magnitude.
...
\node[unobs] (skill) at (1.8, 2.2) {Team Skill\\\emph{(unobserved)}};
\node[confbox] (Conf) at (7.8, 2.2) {%
